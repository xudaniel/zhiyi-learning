...
try{
 await page.goto(url+'#cases');assert.equal(await page.evaluate(()=>page),'cases');
 ...
 await page.reload();assert.equal(await page.evaluate(()=>activeCase),'b');assert.equal(await page.locator('#caseNote').inputValue(),'b：先定位日元壬，再清点。<img src=x onerror=alert(1)>');
 ...
 await page.evaluate(()=>localStorage.clear());await page.reload();await page.locator('input[type=file]').setInputFiles(saved);await page.waitForFunction(()=>state.academy.cases.b?.step===5);assert.deepEqual(await page.evaluate(()=>state.academy.reviews),backup.state.academy.reviews);await page.reload();await go('cases');assert.equal(await page.evaluate(()=>activeCase),'b');
 ...
 for(const route of ['cases','compare','sound','today','ask']){await page.goto(url+'#'+route);assert.equal(await page.evaluate(()=>page),route)}await page.goto(url+'#unknown');assert.equal(await page.evaluate(()=>page),'home');
