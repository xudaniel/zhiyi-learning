...
const EVIDENCE = {
 language:{title:'先熟记基本语言',source:1,location:'第二节口述课 · 一',kind:'口述原话摘录',needle:'天干必须要记住',length:215},
 imagery:{title:'十天干的物象',source:1,location:'第二节口述课 · 二',kind:'口述原话摘录',needle:'（一）甲木与乙木',length:460},
 ...
 count:{title:'先清点五行',source:1,location:'第二节口述课 · 八（四）',kind:'口述原话摘录',needle:'（四）从八个字清点五行',length:300},
 elements:{title:'五行生克',source:0,location:'讲稿总结 · 四',kind:'既有讲稿总结',needle:'相生：木 → 火 → 土 → 金 → 水 → 木',length:135},
 seasons:{title:'月份改变，所需不同',source:1,location:'第二节口述课 · 八（五）',kind:'口述原话摘录',needle:'（五）不同月份的壬水需要不同五行',length:680},
 spring:{title:'二月壬水的口述情境',source:2,location:'第二节口述课 · 八（二）',kind:'口述原话摘录',needle:'你想象现在农历二月份的水',length:760},
 timeline:{title:'大运与流年',source:0,location:'讲稿总结 · 六',kind:'既有讲稿总结',needle:'## 六、大运与流年',length:365},
 clashes:{title:'两排记六冲',source:1,location:'第二节口述课 · 七',kind:'口述原话摘录',needle:'七、十二地支与六冲',length:770},
...
const WEN_TRANSCRIPT='金口诀：文昌诀\n孩子书桌、当官办公桌的重要位置！\n甲乙巳午报君知\n丙戊申宫丁己鸡\n庚猪辛鼠壬逢虎\n癸见卯来文昌齐\n释：甲在巳位，乙在午位；丙戊在申位，丁己在鸡位；庚在亥位，辛在鼠位；壬在虎位，癸在卯位。\n此诀：以日元为主，年干为辅。';
function evidenceText(id){const e=EVIDENCE[id];if(!e)return '';if(e.source===-1)return WEN_TRANSCRIPT;const text=SOURCES[e.source],start=text.indexOf(e.needle);return start<0?'该摘录暂未找到，请回看完整学习资料。':text.slice(start,start+e.length)}
const CASES = [
...
const COMPARE_QUESTION={q:'同样是壬水，为什么两段口述对“土”的讨论不同？',options:['壬水永远必须补土','月份、冷暖和水量条件不同','只要八字没有土就补土'],correct:1,why:'老师按具体月份与自然情境讨论所需。春寒、水多时的“挡水”与夏季水少时的“补水”条件不同，不能抽成同一个固定补法。',source:'seasons',skill:'月份与条件'};
...
function questionById(id){if(id.startsWith('case:')){const [c,n]=id.slice(5).split('-');const item=CASES.find(x=>x.id===c);return item&&caseQuestions(item)[Number(n)]}if(id==='compare:seasons')return COMPARE_QUESTION;if(id.startsWith('sound:'))return SOUND_CHARS.includes(id.slice(6))?soundQuestion(id.slice(6)):null;if(id.startsWith('lesson:')){const [i,j]=id.slice(7).split('-').map(Number);const q=LESSONS[i]?.qs[j];if(q)return {q:q.q,options:q.opts,correct:q.a,why:q.why,source:['language','imagery','imagery','language','elements','roots','day','count','clashes','timeline','directions','wenchang'][i],skill:LESSONS[i].tag}}return null}
...
const FAQ=[
 ...
 {id:'image',q:'甲乙、庚辛、壬癸怎样形象记忆？',keys:['甲乙','庚辛','壬癸','物象','大树'],answer:'按口述记忆法：甲大树、乙花草；庚毛坯金、辛精细金属；壬江海、癸细雨。这些是助记画面。',source:'imagery'},
 ...
 {id:'uncertain',q:'听辨存疑的内容可以直接套用吗？',keys:['听辨','存疑','原话','原录音'],answer:'先保留疑问，回看讲义或核对原录音。存疑句子不作为确定规则；页面会分清口述摘录、已有总结与教学整理。',source:'limits'}
];
